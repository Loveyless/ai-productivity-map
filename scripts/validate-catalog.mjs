...
function validateCatalog(catalog) {
  ...
  if (catalog.tools.length < 20 || catalog.tools.length > 30) {
    errors.push(`starter catalog must contain 20-30 tools; found ${catalog.tools.length}`);
  }
...
async function fetchStatus(url, method) {
  const response = await fetch(url, {
    ...
    headers: {
      'user-agent': 'AI-Productivity-Map-Link-Checker/1.0 (+https://github.com/Loveyless/ai-productivity-map)',
...
console.log(`Catalog OK: ${catalog.tools.length} tools across ${catalog.categories.length} categories.`);
